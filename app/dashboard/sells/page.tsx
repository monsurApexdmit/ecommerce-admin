...
export default function SellsPage() {
    ...
    const filteredSells = sells.filter((sell) => {
        const matchesSearch = sell.customerName.toLowerCase().includes(searchQuery.toLowerCase())
        const matchesStatus = statusFilter === "all" || sell.status === statusFilter
        const matchesMethod = methodFilter === "all" || sell.method === methodFilter

        let matchesDateRange = true
        if (startDate && endDate) {
            const sellDate = new Date(sell.orderTime)
            const start = new Date(startDate)
            const end = new Date(endDate)
            matchesDateRange = sellDate >= start && sellDate <= end
        }
    ...
    const handleStatusChange = (invoiceNo: string, newStatus: Sell["status"]) => {
        setSells(sells.map((sell) => (sell.invoiceNo === invoiceNo ? { ...sell, status: newStatus } : sell)))
    }

    const handleDownloadAllSells = () => {
        const csvContent = [
            ["Invoice No", "Sell Time", "Customer Name", "Method", "Amount", "Status"],
            ...filteredSells.map((sell) => [
                sell.invoiceNo,
                sell.orderTime,
                sell.customerName,
                sell.method,
                sell.amount.toString(),
                sell.status,
            ]),
        ]
            .map((row) => row.join(","))
        ...
        a.href = url
        a.download = "sells.csv"
        a.click()
        window.URL.revokeObjectURL(url)
    }

    const handlePrintInvoice = (sell: Sell) => {
        const printWindow = window.open("", "_blank")
        if (printWindow) {
            printWindow.document.write(`
        <html>
          <head>
            <title>Invoice ${sell.invoiceNo}</title>
            <style>
          ...
          <body>
            <div class="invoice-container">
              <div class="header">
                <div>
                  ...
                  <div>
                    ...
                    <span class="status-badge">${sell.status}</span>
                  </div>
                </div>
              ...
              <div class="invoice-meta">
                <div class="meta-item">
                  <h3>Date</h3>
                  <p>${sell.orderTime.split(" ")[0]} ${sell.orderTime.split(" ")[1]} ${sell.orderTime.split(" ")[2]}</p>
                </div>
                <div class="meta-item">
                  <h3>Invoice No</h3>
                  <p>#${sell.invoiceNo}</p>
                ...
                <div class="meta-item invoice-to">
                  <h3>Invoice To</h3>
                  <p style="font-weight: 600;">${sell.customerName}</p>
    ...
    const handleDownloadInvoice = (sell: Sell) => {
        handlePrintInvoice(sell)
    }

    const handleEmailInvoice = (sell: Sell) => {
        alert(`Email invoice #${sell.invoiceNo} functionality would be implemented here`)
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <Card className="p-6">
                <div className="space-y-4 mb-6">
                    <div className="flex flex-col lg:flex-row gap-4">
                        ...
                        <Button onClick={handleDownloadAllSells} className="bg-emerald-600 hover:bg-emerald-700 whitespace-nowrap">
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b bg-gray-50">
                                ...
                                <th className="text-left py-3 px-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Sell Time</th>
                ...
                {filteredSells.length === 0 && (
                    <div className="text-center py-12">
                        <p className="text-gray-500">No sells found</p>
                    </div>
                )}
            </Card>
            ...
            <PaginationControl
                ...
                totalItems={filteredSells.length}
            />
                    ...
                    {selectedSell && (
                        <div className="space-y-8 py-4">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                                <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <StatusBadge status={selectedSell.status} />
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-900">
                                        {selectedSell.orderTime.split(" ")[0]} {selectedSell.orderTime.split(" ")[1]}{" "}
                                        {selectedSell.orderTime.split(" ")[2]}
                                    </p>
                                </div>
                                <div>
                                    <h3 className="text-xs font-semibold text-gray-600 uppercase mb-2">Invoice No</h3>
                                    <p className="text-sm text-gray-900">#{selectedSell.invoiceNo}</p>
                                </div>
                                <div className="sm:text-right">
                                    <h3 className="text-xs font-semibold text-gray-600 uppercase mb-2">Invoice To</h3>
                                    <p className="text-sm font-semibold text-gray-900">{selectedSell.customerName}</p>
                            ...
                            <div className="bg-gray-50 p-6 rounded-lg">
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
                                    <div>
                                        ...
                                        <p className="text-base font-semibold text-gray-900">{selectedSell.method}</p>
                                    </div>
                                    <div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 pt-4">
                                <Button
                                    onClick={() => handleDownloadInvoice(selectedSell)}
                                ...
                                <Button
                                    onClick={() => handleEmailInvoice(selectedSell)}
                                ...
                                <Button
                                    onClick={() => handlePrintInvoice(selectedSell)}
